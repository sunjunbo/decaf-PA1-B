...
FieldList       :   STATIC Type Id '(' VarList ')' Block FieldList
                ...
                {
                    $$ = $8;
                    $$.fieldList.add(0, new MethodDef(false,true, $3.id, $2.type, $5.varList, Optional.ofNullable(null), $3.pos));
                }
                |   /* empty */
                    {
                        $$ = svFields();
...
Type            :   AtomType ArrayType TLambdas
                    {
                        ...
                        for (int i = 0; i < $2.intVal; i++) {
                            $$ = svType(new TArray($$.type, $1.type.pos));
                        }
                        for (var sv : $3.thunkList)
                        {
                            $$ = svType(new TLambda($$.pos, $$.type, sv.typeList));
                            for (int i = 0; i < sv.intVal; i++) {
                                $$ = svType(new TArray($$.type, $1.type.pos));
                            }
                        }
                    }
...
TLambdas : '(' TypeList ')' ArrayType TLambdas
                    {
                        $$ = $5;
                        var sv = new SemValue();
...
SimpleStmt      :   Var Initializer
                ...
                |
                    VAR Id '=' Expr
                    {
                        $$ = svStmt(new LocalVarDef(new TVar(), $2.id, $2.pos, Optional.ofNullable($4.expr), $2.pos));
                    }
                |   /* empty */
                    {
                        $$ = svStmt(null);
...
Expr            :
                    FUN '(' VarList ')' FUN_Tail
                    {
                        if($5.expr != null)
                            $$ = svExpr(new Lambda($1.pos, $3.varList, $5.expr));
                        else
                            $$ = svExpr(new Lambda($1.pos, $3.varList, $5.block));
                    }
                    | Expr1
                    {
                        $$ = $1;
                ...
                |   Expr ')' ExprT8
                    {
                        ...
                        for (var sv : $3.thunkList) {
                            if (sv.expr != null) {
                                ...
                                $$ = svExpr(new Call(Optional.ofNullable($$.expr), sv.exprList, sv.pos));
                            } else {
...
AfterNewExpr    :   Id '(' ')'
                ...
                |    AtomType  NewArrayOrLambdaRem
                {
                       ...
                       $2.thunkList.remove($2.thunkList.size()-1);
                       for(var sv : $2.thunkList)
                       {
                            if(sv.typeList != null)
                            {
                                $$.type = new TLambda($$.type.pos, $$.type, sv.typeList);
                            }
                            else
                            {
                                for (int i = 0; i < sv.intVal; i++) {
...
NewArrayOrLambdaRem : '(' TypeList ')'  NewArrayOrLambdaRem
                ...
                {
                    $$ = $2;
                    //System.out.println($$.thunkList);
                }
                ;
NewArrayRem : ']' NewArrayOrLambdaRem
                {
                    $$ = $2;
                    ++$$.intVal;
                }
